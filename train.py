...
def train_epoch(feature_extractor, source_classifier, domain_discriminator, prototype_classifier,
    ...
    for (s_data, s_label), (ut_data, _), (lt_data, lt_label), (plt_data, plt_label) in tqdm(itertools.zip_longest(s_iter, ut_iter, lt_iter, plt_iter, fillvalue=(None, None))):
        ...
        source_classification_loss = 0
        adversarial_curriculum_loss = 0
        diverse_curriculum_loss = 0
        prototype_classification_loss = 0
        selfsupervised_clustering_loss = 0
        ...
        if ut_data is not None:
            ...
            # --- 敵対的カリキュラム学習 L_adv ---
            adversarial_curriculum_loss += torch.mean((ut_transfer_scores >= config.w_alpha).float().flatten() * torch.log(torch.clamp(ut_domain_preds, min=eps)).flatten())
            # --- 多様性カリキュラム学習 L_div ---
        ...
        if lt_data is not None:
            ...
            if len(lt_common_domain_preds) >= 0:
                adversarial_curriculum_loss += torch.mean(torch.log(torch.clamp(lt_common_domain_preds, min=eps)))
            # --- 多様性カリキュラム学習 L_div ---
            diverse_curriculum_loss -= (torch.mean(torch.sum(source_classifier(lt_features[private_label_indices]) \
